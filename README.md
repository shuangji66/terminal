...
- **单设备占用** — 一个会话同时**只在一台设备上**可用：另一台设备打开（挂载）该会话时，
  前一端会收到「已被接管」通知（`\x1b]detached\x07` + WS close `4001`），该标签显示
  「会话已在其他设备打开」并可点「重连」夺回；**其他会话的连接不受影响**。被顶掉只是
  解挂载，会话进程与历史继续保留。
...
- **终端字体** — 终端内容默认使用内置的 **Maple Mono CN**（Regular 400，按 unicode-range
  分片按需加载，字体随二进制一起分发、不访问任何 CDN）；设置里可切换为**系统字体**
  （选择只存浏览器本地，选系统字体时不会加载内置字体切片）；界面其余部分一律系统字体。
- **终端配色** — 深色模式黑底绿字（绿色略暗淡，`#2bd957`）；浅色模式米白底黑字；
  顶栏浅色为温和白色，与终端米白错开。
- **移动端辅助功能键** — 两页胶囊按键条：
  **第一页**第一行 ESC | ↑ | Tab | Ctrl | Alt | Shift | **Insert**，第二行 ← | ↓ | →
  与 `/ - = " .`；**第二页**第一行 `! @ # $ % ^ & *`，第二行依次是 , ; [ ] \ ` ( )。
  手机档**点两侧的细长竖条按钮切页**
  （第一页只在右侧显示「›」→ 第二页；第二页只在左侧显示「‹」→ 回第一页，不循环、不滑动切页）；
  **平板档两页并排同时显示**。
- **Shift 上档锁定** — 点一次 Shift 即锁定（按钮变实心品牌色），此后符号键输出上档字符
  （`.→<` `,→>` `/→?` `;→:` `"→'` `[→{` `]→}` `\→|` `-→_` `=→+`），
  **方向键同时上档**：←→ Home/End、↑↓ PageUp/PageDown（键面显示 **HM / ED / PU / PD**）；
  **键面同步显示上档字符/缩写**并变成品牌色以示「已上档」；
  **一直有效到再点一次 Shift 解除**，可连续输入（长按连发同样走上档序列）。
  方向键支持长按连发；修饰键（Ctrl/Alt/Shift）按下后变色，**输入一次键盘输入后自动解除**。
...
- **单挂载点**：一个会话同一时刻只有一个操作端。新挂载顶掉旧端：旧端先收到
  `\x1b]detached\x07`，随后以 WS **close 4001**（reason `detached`）关闭；被顶掉端的
  输入与尺寸请求在服务端被丢弃。前端据此进入 `detached` 状态并**不自动重连**，
  需要用户点「重连」显式夺回。
